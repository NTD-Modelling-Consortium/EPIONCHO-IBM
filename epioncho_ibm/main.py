...
def generate_age_dist(
    ...
    for i in range(limit):
        ...
        death_vector = np.random.binomial(
            n=1, p=(1 / mean_human_age) * delta_time, size=population_size
        )
...
def ep_equilibrium_simulation(
    ...
    current_ages = generate_age_dist(
        params.human_population, params.mean_human_age, params.max_human_age, delta_time
    )
    ...
    worm_mortality_rate = weibull_mortality(
        delta_time, params.mu_worms1, params.mu_worms2, worm_age_categories
    )
