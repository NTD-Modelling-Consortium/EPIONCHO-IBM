...
class State:
    ...
    @classmethod
    def generate_random(
        cls, random_config: RandomConfig, n_people: int, params: Params
    ) -> "State":
        sex_array = (
    ...
    def mf_prevalence_in_population(self: "State", min_age_skinsnip: int) -> float:
        ...
        infected_over_min_age = np.sum(
            np.logical_and(pop_over_min_age_array, self._people.mf_current_quantity > 0)
        )
